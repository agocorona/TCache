...
STM transactions with objects thar synchronize synchronously or asynchronously with
...
@DBRefs@ are univocally identified by its object keys, so they can be compared, ordered, checked for equality, and so on.
...
-- contains the hashtable, last sync time and te list of references to be written in permanent storage
...
--
-- `Asynchronous` mode  invokes `clearSyncCache`. For more complex use of the synchronization
-- please use this `clearSyncCache`.
--
-- When interruptions are  controlled, use `SyncManual` mode and include a call to `syncCache` in the finalization code
